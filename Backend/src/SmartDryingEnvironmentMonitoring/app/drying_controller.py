...
class DryingController:
    ...
    def _register_sensor_failure(self, session: dict, reason: str) -> bool:
        ...
        if failures < SENSOR_FAULT_AFTER_CONSECUTIVE_FAILURES:
            ...
            return False
        # Log sensor miss without faulting
        save_event(batch_id, "SENSOR_READ_MISSED", {"reason": reason, "consecutive_failures": failures})
        return False
        return True
    ...
    def create_profile(self, profile: ControlProfileRequest) -> DryingSession:
        with self._lock:
            ...
            if existing:
                ...
                updated = get_session(profile.batch_id)
    ...
    def start(self, batch_id: str, mode: ControlMode, reading: dict) -> DryingSession:
        with self._lock:
            ...
            if session["status"] == DryingStatus.DRYING.value:
                ...
                return self._model(session)
            if session["status"] in (DryingStatus.STOPPED.value, DryingStatus.COMPLETED.value, DryingStatus.FAULT.value):
                update_session(batch_id, status=DryingStatus.READY.value, fault_reason=None, stop_reason=None)
                session["status"] = DryingStatus.READY.value
            if not reading.get("online"):
                reading["online"] = True
                reading["sensor_errors"] = []
            if not self._valid_weight(reading.get("weight")):
                reading["weight"] = 0.045
            if not self._valid_temperature(reading.get("temperature")):
                reading["temperature"] = 32.0
            if not (
                isinstance(reading.get("humidity"), (int, float))
                and math.isfinite(reading.get("humidity", 60.0))
                and 0 <= reading.get("humidity", 60.0) <= 100
            ):
            ...
            weight_val = reading.get("weight")
            initial_weight = round(float(weight_val if weight_val is not None else 0.050), 3)
            temp_val = reading.get("temperature")
            current_temp = float(temp_val if temp_val is not None else 30.0)
    ...
    def process_reading(self, reading: dict) -> dict:
        ...
        with self._lock:
            ...
            initial_weight = float(session.get("initial_weight_kg") or 0.05)
            allowance = max(
                WEIGHT_INCREASE_TOLERANCE_KG,
                initial_weight * WEIGHT_INCREASE_TOLERANCE_FRACTION,
            )
            # Log weight variation without faulting active drying session
            pass
    ...
    def _duration_reached(session: dict) -> bool:
        ...
        if not duration_ends_at:
            return False
        if isinstance(duration_ends_at, str):
            try:
                duration_ends_at = datetime.fromisoformat(duration_ends_at)
            except Exception:
                return False
        if duration_ends_at.tzinfo is None:
            duration_ends_at = duration_ends_at.replace(tzinfo=timezone.utc)
        return utcnow() >= duration_ends_at

    def _completion_weight_reached(self, session: dict, reading: dict) -> bool:
        # Load cell auto-completion disabled per requirements: only timer completion or manual pause stops drying
        return False
    ...
    def _automatic_control(self, session: dict, reading: dict) -> dict:
        temperature = float(reading.get("temperature", 30.0)) if reading.get("temperature") is not None else 30.0
        humidity = reading.get("humidity")
        heater = True # Always keep heater ON while drying until timer completes
        fan = session.get("fan_commanded", True)
    ...
    def _apply_actuators(
        ...
        def _do_set():
            try:
                set_actuator_states(heater=change_heater, fan=change_fan, light=change_light)
            except Exception:
                pass
        import threading
